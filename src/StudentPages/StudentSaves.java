...
import StudentPagecomponent.CardPanelSaves;
import java.sql.*;
import javax.swing.*;
...
public class StudentSaves extends javax.swing.JPanel {
    
    private String Email;
    
    public StudentSaves(String email) {
        initComponents();
        this.Email = email;
        setOpaque(false);
        init();
    ...
    private void init() {
        ...
        try {
            ...
            String sql = "SELECT denomination, university FROM studentsaves WHERE studentemail='"+this.Email+"'";
            ...
            while(rs.next()) {
                ...
                panel.add(new CardPanelSaves(new Model_CardSaves(new ImageIcon(getClass().getResource("/UniversityImages/"+uni+".jpg")), uni, sch, this.Email)));
            }
            con.close();
            panel.revalidate();
            panel.repaint();
    ...
    private boolean ExistsComponents() {
